...
func left(pos, dimx, dimy int) (neighborPos int, works bool) {
	if pos % dimx == 0 {
		return 0, false
	} else {
		return pos-1, true
...
func dijkstra(matrix []int, dimx, dimy int, start, goal int) []int {
	...
	for ;len(allNodes) > 0; {
		...
		leftNode, leftWorks := left(u,dimx,dimy)
		...
		// fmt.Println("  ", leftNode, leftWorks, matrix[leftNode], dist[u] + int64(matrix[leftNode]))
		...
		if leftWorks == true {
			alt := dist[u] + int64(matrix[leftNode])
			if alt < dist[leftNode] {
				dist[leftNode] = alt
				previous[leftNode] = u
			}
		}
...
func main() {
	...
	previous := dijkstra(matrix, dimx, dimy, start, goal)
